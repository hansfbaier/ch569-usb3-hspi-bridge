 ...
 * 主机可向1号端点突发4包数据后然后从1号端点取走4包数据 The host can burst 4 packets of data to endpoint 1 and then take 4 packets of data from endpoint 1
 * 主机可连续向2号端点下传数据 同时也可连续从2号端点取数据 The host can continuously download data to endpoint 2, and can also continuously fetch data from endpoint 2
 * */
/* Global define */
/* Global Variable */
...
__attribute__((aligned(16))) UINT8 endp0RTbuff[512] __attribute__((section(".DMADATA")));  //端点0数据收发缓冲区 Endpoint 0 data transceiver buffer
...
void EP1_OUT_Callback(void)
{
    ...
    else if(nump == 1) //还可以接收一包 You can also receive a package
    {
        USB30_OUT_ClearIT(ENDP_1);
        USB30_OUT_Set(ENDP_1, ACK, 1);    //able to receive a packet
        USB30_Send_ERDY(ENDP_1 | OUT, 1); //通知主机下发1包 Notify the host to deliver packet 1
    }
    else if(nump == 2)
    {
        USB30_OUT_ClearIT(1);
        USB30_OUT_Set(ENDP_1, ACK, 2);    //能够接收2包 able to receive 2 packets
        USB30_Send_ERDY(ENDP_1 | OUT, 2); //通知主机下发2包 Notify the Host machin to deliver packet 2
    }
    else if(nump == 3)
    {
        USB30_OUT_ClearIT(1);
        USB30_OUT_Set(ENDP_1, ACK, 3);    //能够接收3包 able to receive 3 packets
        USB30_Send_ERDY(ENDP_1 | OUT, 3); //通知主机下发3包 notify the host machine to deliver packet 3
    }
}
