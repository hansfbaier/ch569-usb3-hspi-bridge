...
__attribute__((aligned(16))) uint8_t endp0RTbuff[512] __attribute__((section(".dmadata")));
...
void USB30D_init(FunctionalState sta)
{
    ...
    if(sta)
    {
        ...
        USBSS->UEP1_TX_DMA = (uint32_t)(uint8_t *)in_buf0;
        USBSS->UEP1_RX_DMA = (uint32_t)(uint8_t *)out_buf0;
    }
    else
    {
